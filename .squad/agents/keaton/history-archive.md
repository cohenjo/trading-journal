...
### 2026-06-04 — Next.js 16.2.6 → 16.2.7 Upgrade Analysis

**Requested by:** Jony Vesterman Cohen
**Work:** Analyzed upgrade path for Next.js patch bump prompted by an alert message claiming security urgency.

**Next.js dependency posture:**
- App uses `next@16.2.6` pinned in package.json with `^` range; actual installed is 16.2.6 per package-lock.
- `eslint-config-next` must move in lockstep with `next` — both are at 16.2.6, both have 16.2.7 available.
- Package manager: **npm** (package-lock.json present; no pnpm-lock.yaml or yarn.lock).
- `next dev --turbopack` is active via the `dev` script — turbopack-specific patches ARE relevant.
- Middleware (`src/middleware.ts`) does Supabase session refresh on every request — the server-action forwarding loop fix (#93919) is directly relevant.

**Patch contents (16.2.7):**
1. Docs fixes only
2. Playwright-core internal fix (test harness, not user-facing)
3. Dev-mode hydration failure when page served from HTTP cache ← dev-experience fix
4. `router.query` corruption with basePath + rewrites (N/A — app uses no basePath/rewrites config)
5. Encode non-ASCII characters in cache tags ← could affect Hebrew content if caching ever lands
6. **Server action forwarding loop with middleware rewrites** ← RELEVANT (middleware + server actions)
