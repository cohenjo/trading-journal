...
- Production-hardened Growth Story service (2026-07-25, PR #16). Key pattern: SDK services should return `None` on failure instead of raising — let the endpoint decide the fallback strategy. Added schema validation (`_validate_response`) as a gate before accepting AI output. Retry with simplified prompt on malformed JSON. Graceful fallback to template-based synthesis. Structured logging with `ticker=, model=, duration=, source=` fields. The `source` field in API responses ("ai" vs "template") lets the frontend distinguish AI analysis from fallback data.
