...
class PlanService:
    ...
    def calculate_projection(
        ...
        real_assets_list, unallocated_cash_diff = RealAssetManager.load_real_assets(
            plan_data, finance_snapshot, user_settings
        )
        ...
        for year in range(current_year, end_year + 1):
            ...
            milestone_manager.check_dynamic_milestones(
                year, liq_nw, 0.0
            )  # expenses passed as 0 for now to avoid circular dep
                ...
                for acc in account_manager.accounts:
                    ...
                    if gross_dividend > 0:
                        ...
                        dividend_payouts.append(
                            {"name": f"Dividend: {acc['name']}", "type": "Dividend Income", "gross": gross_dividend}
                        )
            ...
            for item in items:
                ...
                start_y = milestone_manager.get_year_from_condition(
                    item, "start_condition", "start_reference", "start_date"
                )
                ...
                if year >= start_y and year <= end_y:
                    ...
                    if cat == "Asset":
                        ...
                        if is_purchase:
                            ...
                            if financing:
                                down_pct = (
                                    PlanService._safe_float(financing.get("down_payment", 0)) / base_val_raw
                                    if base_val_raw
                                    else 0
                                )
            ...
            for ann in active_annuities:
                ...
                income_details_list.append(
                    {"name": f"Pension: {ann['name']}", "type": "Pension Income", "value": round(val, 2)}
                )
            ...
            if net_flow > 0:
                ...
                # Pass year and current year_taxable_income (acting as base_annual_income for marginal tax)
                _, unallocated_cash, withdrawals, withdrawal_details_list = account_manager.process_deficit(
                    deficit, unallocated_cash, current_year=year, base_annual_income=year_taxable_income
                )

                # Add any tax withheld from withdrawals to the year's total
                for wd in withdrawal_details_list:
                    if wd.get("tax_withheld"):
                        tax_w = wd["tax_withheld"]
                        year_tax_paid += tax_w
                        # We also add the gross withdrawal to taxable income if it's treated as income
                        # For RSU, part is income, part is capital gains, but tax_paid is enough for cashflow.
