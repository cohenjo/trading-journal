...
class MilestoneManager:
        ...
        for m in self.milestones:
            ...
            if m_type == "Custom" or m_type == "Retirement":
                if m.get("date"):
                    ...
                    if m_owner == "Spouse":
                        target_birth_year = PlanInterfaces._safe_int(
                            self.user_settings.get("spouse", {}).get("birthYear", self.birth_year), self.birth_year
                        )
    ...
    def get_year_from_condition(
        self,
        item,
        target_cond_field="start_condition",
        target_ref_field="start_reference",
        target_date_field="start_date",
    ) -> int:
    ...
    def check_dynamic_milestones(self, year: int, liquid_net_worth: float, annual_expenses: float):
        ...
        if fi_milestone:
            multiplier = PlanInterfaces._safe_float(
                fi_milestone.get("details", {}).get("expense_multiplier", 25.0), 25.0
            )
...
class AccountManager:
    @staticmethod
    def load_accounts(
        plan_data: Dict, finance_snapshot: Any, user_settings: Dict, db: Optional[Session] = None
    ) -> List[Dict]:
            ...
            for f_item in snapshot_items:
                ...
                if cat in ["Savings", "Investments", "Cash", "Checking", "Bank", "Liquid", "Retirement"]:
                    ...
                    if "broker" in f_type:
                        ...
                        acc_type = "HSA"
                    elif "pension" in f_type:
                        acc_type = "Pension"
                    elif "savings" in f_type:
                        acc_type = "Savings"

                    snapshot_settings = {}
                    if "draw_income" in f_details:
                        snapshot_settings["draw_income"] = f_details.get("draw_income")
                    if "divide_rate" in f_details:
                        snapshot_settings["divide_rate"] = PlanInterfaces._safe_float(f_details.get("divide_rate"))
                    if "starting_age" in f_details:
                        snapshot_settings["starting_age"] = PlanInterfaces._safe_int(f_details.get("starting_age"))
                    if "monthly_contribution" in f_details:
                        snapshot_settings["monthly_contribution"] = PlanInterfaces._safe_float(
                            f_details.get("monthly_contribution")
                        )
                    if f_details.get("max_withdrawal_rate") is not None:
                        snapshot_settings["max_withdrawal_rate"] = PlanInterfaces._safe_float(
                            f_details.get("max_withdrawal_rate")
                        )
                    if f_details.get("max_withdrawal_cap") is not None:
                        snapshot_settings["max_withdrawal_cap"] = PlanInterfaces._safe_float(
                            f_details.get("max_withdrawal_cap")
                        )

                    # PRIORITY MAPPING
                    if f_item.get("withdrawal_priority") or f_details.get("withdrawal_priority"):
                        snapshot_settings["withdrawal_priority"] = f_item.get("withdrawal_priority") or f_details.get(
                            "withdrawal_priority"
                        )
                    if f_item.get("inflow_priority") or f_details.get("inflow_priority"):
                        snapshot_settings["inflow_priority"] = f_item.get("inflow_priority") or f_details.get(
                            "inflow_priority"
                        )

                    # DIVIDEND DEFAULTS - Only override if explicit in snapshot
                    if "dividend_policy" in f_details:
                        snapshot_settings["dividend_policy"] = f_details.get("dividend_policy")
                    if "dividend_mode" in f_details:
                        snapshot_settings["dividend_mode"] = f_details.get("dividend_mode")
                    ...
                    if div_amount is None and db is not None:
                        try:
                            ...
                            if div_acc:
                                ...
                                d_positions = db.exec(
                                    select(DividendPosition).where(DividendPosition.account == div_acc.name)
                                ).all()
                                if d_positions:
                                    tickers = [p.ticker for p in d_positions]
                                    td_list = db.exec(
                                        select(DividendTickerData).where(DividendTickerData.ticker.in_(tickers))
                                    ).all()
                    ...
                    if div_amount is not None:
                        snapshot_settings["dividend_fixed_amount"] = PlanInterfaces._safe_float(div_amount)

                    if "dividend_growth_rate" in f_details:
                        snapshot_settings["dividend_growth_rate"] = PlanInterfaces._safe_float(
                            f_details.get("dividend_growth_rate")
                        )
                    if "dividend_tax_rate" in f_details:
                        snapshot_settings["dividend_tax_rate"] = PlanInterfaces._safe_float(
                            f_details.get("dividend_tax_rate")
                        )
                    if "dividend_payout_start_condition" in f_details:
                        snapshot_settings["dividend_payout_start_condition"] = f_details.get(
                            "dividend_payout_start_condition"
                        )
                    if "dividend_payout_start_reference" in f_details:
                        snapshot_settings["dividend_payout_start_reference"] = f_details.get(
                            "dividend_payout_start_reference"
                        )
                    ...
                    plan_settings = config.get("account_settings") or config.get("details", {}).get(
                        "account_settings", {}
                    )
                    ...
                    final_inflow_priority = PlanInterfaces._safe_int(snapshot_settings.get("inflow_priority", 100))
                    ...
                    accounts.append(
                        {
                            ...
                            "monthly_contribution": PlanInterfaces._safe_float(
                                merged_settings.get("monthly_contribution", 0.0)
                            ),
                            ...
                            "max_withdrawal_rate": PlanInterfaces._safe_float(
                                merged_settings.get("max_withdrawal_rate", 0.0)
                            )
                            if merged_settings.get("max_withdrawal_rate") is not None
                            else None,
                            "max_withdrawal_cap": AccountManager._convert(
                                PlanInterfaces._safe_float(merged_settings.get("max_withdrawal_cap", 0.0)),
                                currency,
                                main_currency,
                            )
                            if merged_settings.get("max_withdrawal_cap") is not None
                            else None,
                            "savings_goal": AccountManager._convert(
                                PlanInterfaces._safe_float(merged_settings.get("savings_goal", 0.0)),
                                currency,
                                main_currency,
                            )
                            if merged_settings.get("savings_goal") is not None
                            else None,
                            "dividend_policy": merged_settings.get("dividend_policy", "Accumulate"),
                            "dividend_mode": merged_settings.get("dividend_mode", "Percent"),
                            "dividend_fixed_amount": AccountManager._convert(
                                PlanInterfaces._safe_float(merged_settings.get("dividend_fixed_amount", 0.0)),
                                currency,
                                main_currency,
                            )
                            if merged_settings.get("dividend_fixed_amount") is not None
                            else None,
                            "dividend_growth_rate": PlanInterfaces._safe_float(
                                merged_settings.get("dividend_growth_rate", 0.0)
                            ),
                            "dividend_tax_rate": PlanInterfaces._safe_float(
                                merged_settings.get("dividend_tax_rate", 0.0)
                            ),
                            ...
                            "rsu_grants": merged_settings.get("rsu_grants", []),
                            "current_price": PlanInterfaces._safe_float(merged_settings.get("current_price", 0.0)),
                        }
                    )
        ...
        for p_item in items:
            if p_item.get("category") == "Account":
                if not any(a["name"] == p_item["name"] for a in accounts):
                    ...
                    final_withdrawal_priority = PlanInterfaces._safe_int(
                        p_item.get("withdrawal_priority") or acc_set.get("withdrawal_priority", 50)
                    )
                    final_inflow_priority = PlanInterfaces._safe_int(
                        p_item.get("inflow_priority") or acc_set.get("inflow_priority", 100)
                    )
                    ...
                    accounts.append(
                        {
                            ...
                            "name": p_item["name"],
                            "value": AccountManager._convert(
                                PlanInterfaces._safe_float(p_item.get("value", 0)), currency, main_currency
                            ),
                            ...
                            "inflow_priority": final_inflow_priority,
                            "monthly_contribution": PlanInterfaces._safe_float(
                                acc_set.get("monthly_contribution", 0.0)
                            ),
                            ...
                            "max_withdrawal_rate": PlanInterfaces._safe_float(acc_set.get("max_withdrawal_rate", 0.0))
                            if acc_set.get("max_withdrawal_rate") is not None
                            else None,
                            "max_withdrawal_cap": AccountManager._convert(
                                PlanInterfaces._safe_float(acc_set.get("max_withdrawal_cap", 0.0)),
                                currency,
                                main_currency,
                            )
                            if acc_set.get("max_withdrawal_cap") is not None
                            else None,
                            "savings_goal": AccountManager._convert(
                                PlanInterfaces._safe_float(acc_set.get("savings_goal", 0.0)), currency, main_currency
                            )
                            if acc_set.get("savings_goal") is not None
                            else None,
                            "dividend_policy": acc_set.get("dividend_policy", "Accumulate"),
                            "dividend_mode": acc_set.get("dividend_mode", "Percent"),
                            "dividend_fixed_amount": AccountManager._convert(
                                PlanInterfaces._safe_float(acc_set.get("dividend_fixed_amount", 0.0)),
                                currency,
                                main_currency,
                            )
                            if acc_set.get("dividend_fixed_amount") is not None
                            else None,
                            "dividend_growth_rate": PlanInterfaces._safe_float(
                                acc_set.get("dividend_growth_rate", 0.0)
                            ),
                            ...
                            "rsu_grants": acc_set.get("rsu_grants", []),
                            "current_price": PlanInterfaces._safe_float(acc_set.get("current_price", 0.0)),
                        }
                    )
        return accounts
    ...
    def process_growth_and_income(
        self, year: int, unallocated_cash: float, resolved_milestones: Dict[str, int]
    ) -> tuple[float, List[Dict], List[Dict], float]:
        ...
        for acc in self.accounts:
            ...
            if acc.get("type") == "Pension" and acc.get("draw_income") and acc["value"] > 0:
                ...
                if acc.get("owner") == "Spouse":
                    spouse_by = PlanInterfaces._safe_int(
                        self.user_settings.get("spouse", {}).get("birthYear", self.birth_year), self.birth_year
                    )
                ...
                if effective_age >= trigger_age:
                    ...
                    if div_rate > 0:
                        ...
                        self.active_annuities.append(
                            {"name": acc["name"], "payout": annual_payout, "tax_rate": acc.get("tax_rate", 0.0)}
                        )
                        acc["value"] = 0.0
                        continue
    ...
    def process_savings(self, net_flow: float, unallocated_cash: float) -> tuple[float, float, List[Dict]]:
        ...
        if remaining_flow > 0:
            ...
            savings_breakdown.append({"name": "Unallocated Cash", "value": round(remaining_flow, 2), "type": "Cash"})

        return 0.0, unallocated_cash, savings_breakdown

    def process_deficit(
        self, deficit: float, unallocated_cash: float, current_year: int = None, base_annual_income: float = 0.0
    ) -> tuple[float, float, float, List[Dict]]:
        # Returns: (remaining_deficit, new_unallocated_cash, withdrawals_total_gross, withdrawal_details)
        from app.services.tax_calculator import calculate_rsu_withdrawal_effective_tax_rate
        ...
        if unallocated_cash >= deficit:
            withdrawal_details_list.append(
                {"name": "Withdrawal: Unallocated Cash", "type": "Portfolio Withdrawal", "value": round(deficit, 2)}
            )
            ...
            if unallocated_cash > 0:
                ...
                withdrawals_total += unallocated_cash
            ...
            for acc in sorted_accs:
                ...
                if withdraw_amt > 0:
                    ...
                    if acc_type == "RSU":
                        ...
                        if base_price > 0 and current_year is not None:
                            years_passed = current_year - self.current_year
                            growth_rate = acc.get("growth", 0.0)
                        ...
                        if current_year is not None:
                            ...
                            # Wait, effective rate calculation takes Gross Amount.
                            # We have Net Amount = withdraw_amt.
                            # Gross = Net / (1 - EffectiveRate)
                            # We can do an iterative approximation, or just calculate eff_rate on the Net Amount
                            # which gives a very close approximation since effective rate scales relatively flatly.
                    ...
                    withdrawals_total += gross_withdraw_amt

                    if acc_type == "RSU":
                        rsu_withdrawn_this_year += gross_withdraw_amt

                    withdrawal_details_list.append(
                        {
                            "name": f"Withdrawal: {acc['name']}",
                            "type": "Portfolio Withdrawal",
                            "value": round(gross_withdraw_amt, 2),
...
class RealAssetManager:
    ...
    def load_real_assets(plan_data: Dict, finance_snapshot: Any, user_settings: Dict) -> tuple[List[Dict], float]:
        ...
        if finance_snapshot:
            snapshot_data = (
                finance_snapshot.get("data") if isinstance(finance_snapshot, dict) else finance_snapshot.data
            )
    ...
    def get_liquid_assets_value(self, plan_items: List[Dict]) -> tuple[float, float]:
        ...
        for ra in self.real_assets:
            ra_item = next(
                (i for i in plan_items if i.get("name") == ra["name"] and i.get("category") == "Asset"), None
            )
            if ra_item:
