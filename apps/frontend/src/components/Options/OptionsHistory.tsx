...
type Props = {
  initialData: OptionsRecord[];
  onSave: (data: OptionsRecord[]) => void;
};

export default function OptionsHistory({ initialData, onSave }: Props) {
  const [records, setRecords] = useState<OptionsRecord[]>(initialData);
  ...
  const handleUpdate = (year: number, value: string) => {
    // Allow negative values for losing years; only coerce empty/NaN to 0
  ...
  return (
    <div className="bg-slate-900 p-4 rounded-lg border border-slate-800">
      <h3 className="text-lg font-semibold mb-4 text-slate-200">Historical Options Income</h3>
              ...
              <tr key={record.year} className="border-b border-slate-800 hover:bg-slate-800/50">
                ...
                <td className="px-4 py-2">
                    <input 
                        type="number" 
                        value={record.amount} 
                        onChange={(e) => handleUpdate(record.year, e.target.value)}
