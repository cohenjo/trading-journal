...
const formatMoney = (val: number, currency: string) => new Intl.NumberFormat('en-US', {
  style: 'currency',
  currency: currency,
...
export const ProgressTable: React.FC<ProgressTableProps> = ({ data, onEdit, onDelete, currency }) => {
  return (
        ...
        <tbody className="divide-y divide-slate-800">
          {data.map((row) => (
            <tr key={row.date} className="hover:bg-slate-800/50 transition-colors group">
              ...
              <td className="p-4 text-right font-bold text-slate-100">{formatMoney(row.net_worth, currency)}</td>
              <td className="p-4 text-right text-slate-300">{formatMoney(row.total_assets, currency)}</td>
              <td className="p-4 text-right text-red-400">{formatMoney(row.total_liabilities, currency)}</td>
              <td className="p-4 text-right text-blue-300">{formatMoney(row.total_savings, currency)}</td>
              <td className="p-4 text-right text-cyan-300">{formatMoney(row.total_investments, currency)}</td>
