            ...
            <div className="flex gap-8 text-sm">
                <div>
                    ...
                    <span className="text-2xl font-bold bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">
                        {data.length > 0 
                            ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(data[0].net_worth)
                            : '$0'}
                    </span>
                </div>
                <div>
                     ...
                     <span className="text-xl font-semibold text-slate-300">
                        {data.length > 0 
                            ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', compactDisplay: 'short' }).format(data[0].total_assets)
                            : '$0'}
                     </span>
                </div>
                 <div>
                     ...
                     <span className="text-xl font-semibold text-red-400">
                        {data.length > 0 
                            ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', compactDisplay: 'short' }).format(data[0].total_liabilities)
                            : '$0'}
                     </span>
                </div>
            </div>
